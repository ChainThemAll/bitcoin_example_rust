...
fn test() {
    use bip39::Mnemonic;
    let mnemonic=Mnemonic::parse("abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about").expect("parse error");
    let unprefixed_salt = "TREZOR";
    let iteration_count = 2048_usize;
    let mut seed = [0u8; 64];
    pbkdf2(
        mnemonic.word_iter(),
        unprefixed_salt.as_bytes(),
        iteration_count,
        &mut seed,
    );
    assert_eq!(seed, mnemonic.to_seed_normalized(unprefixed_salt))
}
